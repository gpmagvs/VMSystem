                ...
                while (true)
                {
                    try
                    {
                        ...
                        using (var database = new AGVSDatabase())
                        {
                            ...
                            List_TaskAGV= database.tables.Tasks.AsNoTracking().Where(task => (int)task.State < 4 || task.State == TASK_RUN_STATUS.WAIT).Select(task => task.DesignatedAGVName).Distinct().ToList();
                        }
                        if (taskList.Count == 0)
                            continue;
